...
qa:
- q:
  - What should I read about merging fine-tuned models?
  - Is there a standard method for combining several fine-tuned checkpoints into one model?
  - What work introduced trimming and sign election for model merging?
  - Where do I start if I want a multitask model without multitask training?
  answers:
  - where-to-start-on-merging-fine-tuned-models
  - interference-is-the-frame
  - no-retraining
- q:
...
- q:
  ...
  answers:
  - small-changes-are-redundant-in-value-and-in-sign
  - the-hyperparameter-free-recipe-and-what-it-costs
- q:
...
- q:
  ...
  answers:
  - short-of-multitask-training-and-mostly-on-signs
- q:
...
- q:
  ...
  - scaling-and-the-disjoint-mean-dominate-the-ablation
claims:
- id: where-to-start-on-merging-fine-tuned-models
  kind: context
  text: 'If you have several models fine-tuned from the same checkpoint and want one model
    that does all their tasks without retraining, this is the standard reference: it names
    the two things that go wrong when you average weights -- redundant changes and sign disagreement
    -- and gives the three-step recipe, trim then elect then disjoint mean, that most later
    merging work compares against.'
  scope: An entry-point judgement, not something the paper certifies about itself, and "most
    later work compares against" is a statement about the literature rather than a result
    in here. It holds for parameter-space merging of checkpoints that share an architecture
    and a pre-trained initialisation. It is not the place to start on merging differently
    initialised models, on distillation, or on mixture-of-experts routing, which answer the
    same user problem by other means. Everything measured is IA3 on T0-3B, full fine-tuning
    of T5-base and T5-large, and CLIP visual encoders.
- id: interference-is-the-frame
  kind: context
  text: 'The contribution is a frame as much as a method: it treats merging failures as interference
    between task vectors -- redundancy and sign disagreement -- rather than as a property
    of the loss landscape, which makes the fix elementwise and cheap instead of something
    that needs alignment, retraining or access to the training data.'
  scope: 'A claim about framing, not a measured result: the paper contains no sentence certifying
    its own standing. What is demonstrated is narrower -- that trimming and sign election
    improve merged accuracy across five settings, and that both phenomena are present in the
    task vectors. Loss-landscape and permutation-alignment accounts of merging predate this
    work and are not compared against here, and "interference" is used narrowly for these
    two effects, not for task interference during multitask training.'
- id: no-retraining
...
- id: small-changes-are-redundant-in-value-and-in-sign
  text: 'The small entries of a task vector carry neither its magnitude nor its direction:
    trimming to the top 20% by magnitude and resetting the other 80% to the pre-trained value
    leaves that task''s own performance essentially unchanged, and flipping the signs of the
    bottom 80% barely moves it while flipping the signs of the top 20% degrades performance
    monotonically in the flip probability.'
  scope: 'Both halves are averages over the eleven IA3 task-specific models, each trimmed
    or perturbed and then evaluated on its own task, so they are statements about redundancy
    inside a single fine-tuning run, established before any merging happens; the sign experiment
    is averaged over three runs and also reported at a 30/70 split. The sweep over k is what
    picks 20%. The retention curve for merged models runs the other way: as k grows past 20
    the merged performance falls and then flattens while sign conflicts rise toward 80% of
    parameters, so 20% is the point where those two curves meet in this setting rather than
    a universally safe density, and the paper notes it depends on how parameter values are
    distributed.'
  evidence: Section 3, Section 7.2, Appendix B.2, Appendix B.3, Appendix C.3
...
- id: short-of-multitask-training-and-mostly-on-signs
  text: 'The merged model stays well short of both a jointly trained multitask model and the
    individual fine-tuned models: 73.6 against 88.9 and 90.5 on ViT-B/32, 76.9 against 88.1
    and 88.8 on T5-large, 66.4 against 73.1 and 71.4 on IA3. Most of that gap is sign estimation
    -- substituting a trained multitask model''s sign vector for the elected one lifts IA3
    merging from 66.4 to 72.0.'
  scope: 'The comparison is the same table as the headline gains, read down the column instead
    of across the row, and the paper states the gap as a limitation rather than hiding it.
    What it does not settle is whether the gap is intrinsic to merging or an artefact of fixed
    hyperparameters, since the multitask rows are trained models with their own tuning. The
    72.0 is an oracle and a ceiling: obtaining that sign vector means training the multitask
    model the merge was supposed to replace. The paper''s cheap approximation -- a multitask
    IA3 model trained on 32 validation examples per task, then its signs taken -- reaches
    67.7 initialised from the mean of the merged models and 66.5 from scratch, about a fifth
    of the 5.6 points. Single runs, IA3 only.'
  evidence: Section 6, Section 7.4, Appendix A, Appendix B.1, Appendix C.1
...
- id: the-hyperparameter-free-recipe-and-what-it-costs
  text: 'The k=20, lambda=1 recipe was tuned, just not on the settings it is reported for:
    ...
    3.0, found k=20 best and 0.9, 1.0 and 1.1 equivalent, then applied that unchanged to the
    unseen T5 and ViT settings. Setting lambda to 1 is also the ablation''s largest single
    drop, 2.5 points on T5-base validation data.'
  scope: 'So the no-validation numbers are a genuine transfer test for T5 and ViT and not
    one for IA3, which is why the IA3 no-validation cells are blank; task arithmetic gets
    its own published default of lambda=0.4 in the same comparison. Identifying the ablation
    row with the recipe is a derivation from the row''s own definition, and the two experiments
    sit in different sections on different splits, so read the 2.5 as an indication of the
    cost rather than a measurement of it -- consistent with the recipe''s one negative cell
    being on T5-base. The offsetting fact is that TIES is far less lambda-sensitive than task
    arithmetic: across the values tried its accuracy spans roughly 68 to 75 where task arithmetic''s
    spans 55 to 75.'
  evidence: Appendix C.4, Section 5, Section 7.3, Appendix B.2, Appendix C.5
...
- id: how-it-was-run
  ...
  scope: Language results use rank classification -- the model's log probabilities for each
    ...
    with early stopping and bfloat16; the IA3 models used batch size 16 and patience 10, with
    no scheduler and no weight decay. Everything ran on 48GB A6000 GPUs, single-task models
    taking 15 minutes to 2 hours and the multitask reference up to 24 hours on four GPUs,
    while a merge plus evaluation takes under 2 minutes outside the template-heavy IA3 protocol.
