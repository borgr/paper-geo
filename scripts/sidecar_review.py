...
def review_page(papers: list[dict]) -> str:
    ...
    outdated = [s for s in on_disk if s not in keep]
    fresh.sort(key=lambda s: -((by_slug.get(s) or {}).get("citations") or 0))
    done = [d for d in (checked(s) for s in fresh) if isinstance(d, dict)]

    out = _page_top(done, by_slug)
    for d in done:
        out += _paper(d, by_slug.get(d["slug"]) or {})
    out += _published(by_slug)
    out += _outdated(outdated)
    return "\n".join(out) + "\n"


def _page_top(done: list[dict], by_slug: dict) -> list[str]:
    """The document head, the count, and the contents list over the drafts below it."""
...
def _paper(d: dict, p: dict) -> list[str]:
    """One draft: what to weigh before accepting it, then the answers, then what dangles."""
    slug = d["slug"]
    ...
    return out + _answers(d, p) + _routes(d) + _dangling(d) + ["</div>"]


def _claim(c: dict, slug: str, p: dict) -> list[str]:
    """One claim, with the two things a reader has to judge and every check against it."""
...
def _answers(d: dict, p: dict) -> list[str]:
    """One question, then the claims published as its answer, then the next question.

    A claim is drawn once, under the first question that points at it, and a later question
    carrying it gets a one-line link back. Drawing it again would make a page of
    near-identical blocks differing by one line, and sorting by a claim's *first* question
    cannot group its second, so the same question would reappear far apart.
    """
    slug = d["slug"]
...
def _routes(d: dict) -> list[str]:
    """Every phrasing of every question, by the route it is meant to catch."""
    if not d["qa"]:
        return []
...
def _dangling(d: dict) -> list[str]:
    """The misreadings and the terminology, each with the note about reading alone.

    Both are published as standalone fragments -- a misreading as its own list item, a term
    as a `DefinedTerm` with nothing beside it -- so the note goes inline, under the words
    that dangle.
    """
    out = []
    if d["misreadings"]:
...
def _published(by_slug: dict) -> list[str]:
    """The accepted sidecars, for the other reason to open this page.

    Not "what must I check" but "what does an accepted sidecar actually look like". They
    are already rendered into the site, so this links the built page rather than restating
    it here.
    """
...
def _outdated(slugs: list[str]) -> list[str]:
    """The drafts written against rules that have since changed, named so nobody reads one."""
    if not slugs:
        return []
    out = [f"<h2>{len(slugs)} stale draft{'s' if len(slugs) > 1 else ''} — do not "
           f"read</h2>",
    ...
    out += [f"<li class=id>{e(s)}</li>" for s in slugs]
    out.append("</ul>")
    return out
...
def review(papers: list[dict]) -> None:
    ...
    if yours:
        ...
        for f in rows:
            ...
            for finding in errs + quality:
                counts[rule_of(finding)] += 1
        if counts:
