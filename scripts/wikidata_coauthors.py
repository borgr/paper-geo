...
def _coauth_intro() -> list[str]:
    """What an author name string is and why resolving it is the whole point of the page."""
    return ["# Wikidata co-authors", "",
...
def _coauth_venues(rows_: list[dict]) -> list[str]:
    """Where each paper was published, grouped by venue, plus the volumes to pick by hand."""
...
def _coauth_fills(rows_: list[dict]) -> list[str]:
    """The two fields `--apply` fills in alongside the authors, or nothing."""
    ...
    return [f"## Language and full text ({n_lang + n_url})", "",
...
def _coauth_settled(rows_: list[dict], qs_path: str | None) -> list[str]:
    """The co-authors an ORCID or a DBLP page settled, which `--apply` writes unattended."""
    ...
    if not n_edits:
        return L + [fill("None left. Every co-author an ORCID or a DBLP page could reach is "
                         "already resolved."), ""]
...
def _coauth_remaining(rows_: list[dict]) -> list[str]:
    """The strings no record settles, each linked into Author Disambiguator."""
    ...
    if not todo:
        return L + ["None. Every author string on every item is resolved.", ""]
...
def write_page(rows_: list[dict], qs_path: str | None) -> str:
    """`tasks/wikidata_coauthors.md`, one section per kind of evidence. Returns its path.

    The order is what the reader does in order: the venues and the settled authors go out in
    one `--apply`, and only the last section is one paper at a time.
    """
    path = os.path.join(TASKS, "wikidata_coauthors.md")
    write_task(path, _coauth_intro() + _coauth_venues(rows_) + _coauth_fills(rows_)
               + _coauth_settled(rows_, qs_path) + _coauth_remaining(rows_))
    return path
