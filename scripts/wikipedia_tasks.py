...
def named(surname: str) -> list[dict]:
    """Articles whose text names the author, each carrying the line that names them."""
    out = []
...
def coined(papers: dict, floor: int) -> tuple[list, list]:
    """Terms coined here, split by whether anyone else has written them up.

    A check is `(term, its most-cited paper, the articles to read)`. An absent term is
    `(term, paper)` and is deliberately not actionable. Below `floor` citations a term is
    in neither list, since nobody would plausibly have written about it.
    """
    checks, absent = [], []
    for term, slugs in sorted(sidecar_terms().items()):
        cands = [papers[sl] for sl in slugs if sl in papers]
        p = max(cands, key=lambda x: x.get("citations") or 0) if cands else None
        if not p or AMBIGUOUS.match(term) or (p.get("citations") or 0) < floor:
            continue
    ...
    return checks, absent


def fields(keywords: list[str]) -> list[tuple[str, dict | None, str]]:
    """Each keyword with the article titled exactly that, or the nearest field article.

    `(keyword, article or None, nearest title or "")`. Both empty says nothing in this
    field is close to the keyword, which is an answer rather than a gap.
    """
    out = []
    for kw in keywords:
        art = exists(kw) or exists(kw[0].upper() + kw[1:])
        near = [] if art else search(kw, limit=1)
        out.append((kw, art, near[0]["title"] if near and in_domain(near[0]["title"]) else ""))
    return out


def _row(h: dict, pad: str = "") -> list[str]:
    """A checkbox linking the article and its talk page, and the line the article states."""
    q = urllib.parse.quote(h["title"].replace(" ", "_"))
    L = [f"{pad}- [ ] [{h['title']}](https://en.wikipedia.org/wiki/{q}) — "
         f"[talk](https://en.wikipedia.org/wiki/Talk:{q})"]
    return L + [f"{pad}  > {h['says']}"] * bool(h.get("says"))
...
def _named_section(mine: list, surname: str) -> list[str]:
    """Being an author makes you the best-placed person to notice a misstatement and the
    worst-placed one to fix it, so every row here is a talk-page item."""
    L = [f"## Articles that mention {surname} ({len(mine)})", ""]
    if not mine:
        return L + ["None found. That is the expected state and not a gap to close.", ""]
...
def _coined_section(checks: list) -> list[str]:
    """Independent uptake is the whole qualification, and what makes each row a check
    rather than a plug."""
...
def _fields_section(rows: list) -> list[str]:
    """The COI-free contribution, and the only section that helps the topic questions a
    paper page cannot answer."""
    ...
    for kw, art, near in rows:
        if art:
            ...
            L.append(f"- *{kw}* — no article under that title; the nearest field article is "
                     f"[{near}](https://en.wikipedia.org/wiki/"
                     f"{urllib.parse.quote(near.replace(' ', '_'))})")
        else:
            L.append(f"- *{kw}* — no article, and nothing in this field close to it")
    return L


def _absent_section(absent: list) -> list[str]:
...
def _state(mine: list, checks: list, absent: list) -> dict:
    """What `update.py` needs for the worklist section without re-running the ~100 API
    calls. `tasks/wikipedia.md` stays the full read."""
    return {"checks": [{"term": t, "citations": p.get("citations") or 0,
                        "articles": [{"title": h["title"], "says": h.get("says") or ""}
                                     for h in found]}
                       for t, p, found in checks],
...
def main() -> None:
    ...
    args = ap.parse_args()
    ident = load_config()["identity"]
    surname = ident["name"].split()[-1]

    mine = named(surname)
    checks, absent = coined({p["slug"]: p for p in read_papers()}, args.min_citations)
    rows = fields(ident.get("keywords") or [])
    ...
    write_json(STATE, _state(mine, checks, absent), indent=1)
