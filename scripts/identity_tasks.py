...
def _jref_split(papers: list[dict]) -> tuple[list, list]:
    """The papers with a field to fill in, as (ready, held back), most cited first.

    Each row is `(paper, journal_ref, doi)` with the fields already on the listing blanked, so
    a paper carrying one of the two still reaches the list for the other.

    Held back: a paper with no publisher DOI whose venue year has not passed. arXiv's help page
    is explicit that "to appear in" is not an appropriate journal reference, and a minted DOI is
    the cheapest proof the version of record exists rather than being scheduled. Weak in the
    safe direction -- it holds back a published paper sometimes, and the section says so, but
    never puts a promise on a listing.
    """
...
def _jref_preamble(ready: list, wait: list, subs: dict) -> list[str]:
    """What the three fields are, what filling them in does not cost, and why no script can."""
...
def _jref_block(n: int, p: dict, jr: str, doi: str, subs: dict) -> list[str]:
    """One paper: a heading, the link to its own form, and the fields to paste.

    Only the fields that are actually empty, and the one that is not gets a line saying what is
    already there -- otherwise the form and this file disagree and the reader has to work out
...
def _jref_ready(ready: list, subs: dict) -> list[str]:
    """The papers to fill in now, highest citations first."""
    ...
    for i, row in enumerate(ready, 1):
        L += _jref_block(i, *row, subs)
    return L


def _jref_wait(wait: list, subs: dict) -> list[str]:
    """The papers held back for want of a minted DOI, with their values anyway."""
    if not wait:
        return []
...
_JREF_TAIL = ["## After filling these in", "",
...
def arxiv_jref(papers: list[dict], subs: dict) -> tuple[str, int, int]:
    """`tasks/arxiv_jref.md`: the papers whose arXiv listing does not say where they appeared.

    Returns the path, how many are ready to fill in, and how many are held back.
    ...
    the same form.
    """
    ready, wait = _jref_split(papers)
    path = os.path.join(TASKS, "arxiv_jref.md")
    write_task(path, _jref_preamble(ready, wait, subs) + _jref_ready(ready, subs)
               + _jref_wait(wait, subs) + _JREF_TAIL)
...
def main() -> None:
    ...
    jref, n_jref, n_wait = arxiv_jref(papers, subs)
