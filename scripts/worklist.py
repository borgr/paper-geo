...
def _drafts_on_disk() -> list[str]:
    """Every slug with a draft file, whether or not the rules have moved under it."""
    return sorted(os.path.basename(f)[:-3] for f in
                  glob.glob(os.path.join(DATA, "sidecars", "drafts", "*.md")))


def _review_block(drafted: list[str], by_slug: dict, papers: list[dict]) -> list[str]:
    """The drafts that can be accepted as they stand, and the page to read them on.

    Writes that page, so the link is this run's.
    """
...
def _undrafted_block(todraft: list[dict], papers: list[dict],
                     stale_drafts: list[str]) -> list[str]:
    """Papers with no draft an agent could still write, and how to ask for a batch."""
...
def sidecar_drafts(papers: list[dict]) -> list[str]:
    ...
    draft is minutes, writing one from a blank file is not.
    """
    from sidecar_io import held, spec_sha
    on_disk = _drafts_on_disk()
    ...
    # It belongs with the undrafted papers, because the remedy is identical -- re-run the
    # drafter -- and giving it a heading of its own would report the same seventeen papers
    # twice under two different counts.
    keep = held(spec_sha())
    drafted = [s for s in on_disk if s in keep]
    stale_drafts = [s for s in on_disk if s not in keep]
    todraft = [p for p in papers
               if not has_live_sidecar(p["slug"]) and p["slug"] not in set(drafted)]
    L = []
    if drafted:
